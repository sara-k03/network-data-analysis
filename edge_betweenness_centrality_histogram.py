...
# --- Compute edge BC for each a-value ---
all_bc = {}
...
for a, color in zip(a_values, colors):
    ...
    probability = counts / counts.sum()                        # req. 2: normalize

    ax.plot(bin_centers, probability, color=color,             # req. 3: line graph
            linewidth=1.5, label=f"α={a:.2f}")

ax.set_xlim(global_min, global_max)                            # req. 4: shared x range
# ax.set_yscale('log')
